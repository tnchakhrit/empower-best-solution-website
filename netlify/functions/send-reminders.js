        ...
        for (const threshold of dueThresholds) {
          ...
          if (contact.email) {
            await sendEmail(
              contact.email,
              isOverdueUpdate
                ? `แจ้งเตือน: อัปเดตวันที่ใบ ร.1 ฉบับใหม่ - ${building.building_name}`
              ...
              isOverdueUpdate
                ? `
                  <div style="font-family:sans-serif; line-height:1.7; color:#333;">
                    <h2 style="color:#1E3A28;">อัปเดตข้อมูลอาคาร</h2>
                    <p>เรียนคุณ ${contact.name},</p>
                    <p>อาคาร <strong>${building.building_name}</strong> ครบกำหนดตรวจสอบอาคารรอบก่อนหน้ามาระยะหนึ่งแล้ว (วันหมดอายุเดิม: ${building.inspection_expiry_date}) คาดว่าท่านน่าจะดำเนินการตรวจสอบและได้รับใบ ร.1 ฉบับใหม่เรียบร้อยแล้ว</p>
                    <p>รบกวนแจ้งวันหมดอายุใหม่ (ตามใบ ร.1 ฉบับล่าสุด) กลับมาที่เรา เพื่อให้ระบบแจ้งเตือนของท่านทำงานได้อย่างถูกต้อง และช่วยให้ท่านวางแผนตรวจสอบอาคารครั้งถัดไปได้ทันเวลาค่ะ</p>
                    <p><a href="${updateFormUrl}" style="color:#1E3A28; font-weight:600;">แจ้งวันหมดอายุใหม่</a></p>
            ...
            await pushLineMessage(contact.line_user_id, [
              {
                ...
                text: isOverdueUpdate
                  ? `อัปเดตข้อมูลอาคารกันหน่อยค่ะ 📋\n\n` +
                    `อาคาร: ${building.building_name}\n` +
                    `ครบกำหนดตรวจสอบรอบก่อนหน้ามาสักพักแล้ว (วันหมดอายุเดิม: ${building.inspection_expiry_date}) คาดว่าตอนนี้น่าจะได้รับใบ ร.1 ฉบับใหม่แล้ว\n\n` +
                    `รบกวนแจ้งวันหมดอายุใหม่กลับมาด้วยนะคะ เพื่อให้ระบบแจ้งเตือนทำงานได้ถูกต้อง และช่วยวางแผนตรวจสอบอาคารครั้งถัดไปให้ทันเวลาค่ะ`
                  : `แจ้งเตือนล่วงหน้า ⏰\n\n` +
                    `อาคาร: ${building.building_name}\n` +
